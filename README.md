<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/yamlquill-dark.png" />
    <source media="(prefers-color-scheme: light)" srcset="assets/yamlquill-light.png" />
    <img alt="yamlquill logo" src="assets/yamlquill-light.png" width="600" />
  </picture>
</p>
...
- **YAML Validation Before Save**: Every file save re-parses the generated YAML to verify it's valid before writing to disk. If serialization produces invalid YAML (indicating a bug), the save fails with a clear error message instead of corrupting your data.
...
- **Optional Backups**: Enable `create_backup: true` in your config to automatically create `.bak` files before saving (e.g., `file.yaml.bak`).
...
YAMLQuill is a Rust-based terminal application for viewing and editing YAML files in a structured, tree-like format. It provides an intuitive vim-style interface for navigating and manipulating complex YAML documents directly in the terminal.

Based on the architecture of [JSONQuill](https://github.com/joeygibson/jsonquill), YAMLQuill brings the same powerful editing experience to YAML files with support for YAML-specific features.
...
- **indexmap**: Ordered key-value maps
...
# Specify theme
yamlquill --theme default-light file.yaml

# Pipe YAML from stdin
cat file.yaml | yamlquill
echo 'name: example\ncount: 42' | yamlquill

# Fetch and edit YAML from an API
curl https://api.example.com/data.yaml | yamlquill

# Open gzip-compressed YAML files (transparent decompression)
yamlquill data.yaml.gz
...
yamlquill config.yaml

# Each document starts collapsed showing a preview
# Press l or → to expand a document and see its contents
# Edit fields within expanded documents normally
# Press h or ← to collapse a document back to preview
```
...
YAMLQuill supports vim-style marks and jump list navigation:
...
YAMLQuill supports vim-style named registers for managing multiple clipboards:
...
**Multi-line string editing (Planned - Phase 4):**
- `Shift+Enter` will insert a newline within the string
- `Enter` commits the edit
- Preserves literal (`|`) or folded (`>`) style
...
### YAMLPath Search (Structural Search)

YAMLPath queries allow you to search by structure rather than text:

| Command | Action | Example |
|---------|--------|---------|
| `:path <query>` | YAMLPath structural search | `:path $.config.database.host` |
| `:jp <query>` | Short alias for `:path` | `:jp $..port` |

**Supported YAMLPath Syntax:**
...
```bash
:path $.config.servers[*].host    # All server hosts
:jp $..timeout                    # All timeout fields anywhere
:path $.users[0:3]                # First 3 users
:path $.database['host','port']   # Multiple properties
:jp $.services[*].enabled         # All enabled flags
```

After executing a YAMLPath search, use `n` to navigate through matches just like text search.
...
| `:format` | Reformat document | Apply YAML formatting (2-space indent, multi-line) |
...
| `:set save` | Save settings to config | Write current settings to `~/.config/yamlquill/config.toml` |
| `:path <query>` | YAMLPath structural search | e.g., `:path $.config.database` |
| `:jp <query>` | Short alias for `:path` | e.g., `:jp $..host` |
...
When adding or editing values, YAMLQuill automatically detects the type:

- `true` / `false` → Boolean
- `null` → Null
- `42` / `3.14` / `-1.5` → Number (Integer or Float)
- Anything else → String

Examples:
- Type `hello` → Stored as string `"hello"`
- Type `42` → Stored as integer `42`
- Type `3.14` → Stored as float `3.14`
- Type `true` → Stored as boolean `true`

## Themes

YAMLQuill includes 15 built-in color themes. Use `:theme` to list all available themes, or `:theme <name>` to switch.
...
YAMLQuill supports a configuration file at `~/.config/yamlquill/config.toml`.
...
# YAML validation strictness: "strict", "permissive", or "none" (default: "strict")
validation_mode = "strict"

# Create backup files (e.g., file.yaml.bak) before saving (default: false)
create_backup = false
...
The `scripts/release-notes` script generates GitHub release notes:

```bash
# Generate release notes (auto-detects current version from Cargo.toml)
./scripts/release-notes 0.2.0

# Generate release notes for specific version range
./scripts/release-notes 0.2.0 0.1.0

# Save to file
./scripts/release-notes 0.2.0 > release-notes-v0.2.0.md
```
